  ...
  fulfilments(createdOn: {from: $from, to: $to}, status: "Enviado al Cliente", after: $after) {
    ...
    edges {
      ...
      node {
        ...
        articles("Enviado al Cliente",first:100) {
          edges {
            node {
              id
              status
...
#Cursor	Fulfilment ID	Fulfilment Status	fromLocation	order ref	order id	order status	Article ID	Article Status	Estafeta number
...
def get_page_of_data(after):
    ...
    client.inject_token('bearer 4f3afd3b-15ba-4929-a7fe-b331b6a0fa2d')

    res = client.execute(page_query,{'after': after, 'from':'2020-05-01', 'to':'2021-01-25'})
    #print(res)
...
def get_all_data(cursor="Y3Vyc29yOi0tLTE2MDgxMjNfXzE2MTAzODg5Njk5NTc="):
    global data
