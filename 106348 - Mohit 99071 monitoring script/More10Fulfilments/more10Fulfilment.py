...
query($after: String, $first: Int){
  orders(createdOn:{
    from:"2020-10-05T00:00:00.192Z" to:"2020-10-05T23:59:59.192Z"
...
with open('output.csv','a') as f, open('cursor.csv','a') as c:
    c.write('S.No, Order Id, cursor'+'\n')
    f.write('Order Id, Fulfilment ID, Fulfilment Status, Created On, SKU Ref, SKU ID, Requested Quantity, Filled Quantity, Rejected Quantity'+'\n')
                    ...
                    if (o>10):
                        for l in range(0,o):
                            ...
                            for mm in range(0,p):
                                ...
                                f.write(','+response['data']['orders']['edges'][j]['node']['fulfilments']['edges'][l]['node']['status'])
                                f.write(','+response['data']['orders']['edges'][j]['node']['fulfilments']['edges'][l]['node']['createdOn'])
                                f.write(','+response['data']['orders']['edges'][j]['node']['fulfilments']['edges'][l]['node']['items']['edges'][mm]['node']['ref'])
                                f.write(','+response['data']['orders']['edges'][j]['node']['fulfilments']['edges'][l]['node']['items']['edges'][mm]['node']['id'])
                                f.write(','+str(response['data']['orders']['edges'][j]['node']['fulfilments']['edges'][l]['node']['items']['edges'][mm]['node']['requestedQuantity']))
                                f.write(','+str(response['data']['orders']['edges'][j]['node']['fulfilments']['edges'][l]['node']['items']['edges'][mm]['node']['filledQuantity']))
                                f.write(','+str(response['data']['orders']['edges'][j]['node']['fulfilments']['edges'][l]['node']['items']['edges'][mm]['node']['rejectedQuantity']))
                                f.write('\n')
