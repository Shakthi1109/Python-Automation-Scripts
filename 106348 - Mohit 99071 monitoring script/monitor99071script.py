...
query($after: String, $first: Int){
  ...
  from : "2020-10-01T00:00:00.000Z", to : "2020-12-01T23:59:59.129Z"
...
with open('output.csv','a') as output, open('cursor.csv','a') as c:
    c.write('S.No, Order Id, cursor'+'\n')
    output.write('Order Id, Fulfilment ID, Fulfilment Status, Created On, SKU Ref, SKU ID, Requested Quantity, Filled Quantity, Rejected Quantity'+'\n')
        ...
        if response is not None:
            with open('output.csv','a') as output, open('cursor.csv','a') as c:
                ...
                for j in range (0,m):
                    ...
                    n=len(response['data']['fulfilments']['edges'][j]['node']['items']['edges'])
                    if(n>10):
                        for k in range (0,n):
                            output.write(response['data']['fulfilments']['edges'][j]['node']['order']['id'])
                            output.write(','+response['data']['fulfilments']['edges'][j]['node']['id'])
                            output.write(','+response['data']['fulfilments']['edges'][j]['node']['status'])
                            output.write(','+response['data']['fulfilments']['edges'][j]['node']['createdOn'])
                            output.write(','+response['data']['fulfilments']['edges'][j]['node']['items']['edges'][k]['node']['ref'])
                            output.write(','+response['data']['fulfilments']['edges'][j]['node']['items']['edges'][k]['node']['id'])
                            output.write(','+str(response['data']['fulfilments']['edges'][j]['node']['items']['edges'][k]['node']['requestedQuantity']))
                            output.write(','+str(response['data']['fulfilments']['edges'][j]['node']['items']['edges'][k]['node']['filledQuantity']))
                            output.write(','+str(response['data']['fulfilments']['edges'][j]['node']['items']['edges'][k]['node']['rejectedQuantity']))
                            output.write('\n')
